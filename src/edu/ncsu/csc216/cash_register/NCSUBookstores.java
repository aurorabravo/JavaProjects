...
public class NCSUBookstores {
	...
	public void userInterface() {
		...
		while (true) {
			...
			if (letter.equalsIgnoreCase(QUIT_STRING)) {
				System.exit(0);
			} else if (letter.equalsIgnoreCase(PURCHASE_STRING)) {
				System.out.println();
				handlePurchase();
			} else if (letter.equalsIgnoreCase(REFUND_STRING)) {
				System.out.println();
				handleRefund();
			} else if (letter.equalsIgnoreCase(BALANCE_STRING)) { 
				System.out.println();
				handleBalance();
			} else {
				//invalid option 
	...
	public void handleRefund() {
		...
		while (true) {
			...
			while (itemMenuNumber < BB_CAP_OPTION || itemMenuNumber > BACK_OPTION) {
				System.out.println("Invalid option. \n");
				itemMenu();
				while (!console.hasNextInt()) {
					System.out.println("Invalid option.");
	...
	public void handlePurchase() {
		...
		try {			
			...
			switch (itemMenuNumber) {
				case BB_CAP_OPTION:
					change = register.processPurchase(BB_CAP_PRICE, processPayment());
					break;
				case TSHIRT_OPTION:
					change = register.processPurchase(TSHIRT_PRICE, processPayment());
					break;
				case SWEATSHIRT_OPTION:
					change = register.processPurchase(SWEATSHIRT_PRICE, processPayment());
					break;
				case BABYDOLL_OPTION:
					change = register.processPurchase(BABYDOLL_PRICE, processPayment());
					break;
				case PENCIL_OPTION:
